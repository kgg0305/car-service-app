    ...
    switch (action.type) {
        case INIT:
            return {
                ...
                lineupBodyList: action.payload.lineupBodyList.map(lineupBody => (
                    {
                        ...lineupBody,
                        is_use: action.payload.bodyInfo.model_lineup_ids.split(',').some(item => item === lineupBody.idx.toString()) ? '0' : '1'
                    }
                )),
                colorBodyList: action.payload.colorBodyList.map(colorBody => (
                    {
                        ...colorBody,
                        is_use: action.payload.bodyInfo.model_color_ids.split(',').some(item => item === colorBody.idx.toString()) ? '0' : '1'
                    }
                )),
