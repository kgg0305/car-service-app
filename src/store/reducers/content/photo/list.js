...
const prefix = "content/photo/list/";
...
export const init = () => async (dispatch) => {
  try {
    const initDataSource = await photoService.getList(0);
    const dataLength = await photoService.getCount();
...
export const showMore = () => async (dispatch, getState) => {
  try {
    ...
    const offset = state.photoList.offset + 10;
    const initDataSource = await photoService.getList(offset);
...
export const publish = (idx, dataSource) => async (dispatch) => {
  try {
    ...
    await photoService.update(bodyInfo);
