...
import {
  ...
  showConfirm,
  remove,
} from "../../../store/reducers/content/movieRank/edit";
...
function Edit() {
  ...
  const { redirectTo, validation, confirm, bodyInfo, contentBodyList } =
    useSelector((state) => ({
      redirectTo: state.movieRankEdit.redirectTo,
      validation: state.movieRankEdit.validation,
      confirm: state.movieRankEdit.confirm,
      bodyInfo: state.movieRankEdit.bodyInfo,
      contentBodyList: state.movieRankEdit.contentBodyList,
    }));
    ...
    <>
      <Space direction="vertical" size={18} className="main-layout">
        ...
        <Space direction="vertical" size={11}>
          <Row justify="middle">
            <Col>
              <label className="main-header-title">동영상 인기순위 수정</label>
            </Col>
            <Col flex="auto" />
            <Col>
              <Space size={10}>
