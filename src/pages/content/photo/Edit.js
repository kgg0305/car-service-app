...
import {
  ...
  showConfirm,
  remove,
} from "../../../store/reducers/content/photo/edit";
...
function Edit() {
  ...
  const { redirectTo, validation, confirm, bodyInfo, contentBodyList } =
    useSelector((state) => ({
      redirectTo: state.photoEdit.redirectTo,
      validation: state.photoEdit.validation,
      confirm: state.photoEdit.confirm,
      bodyInfo: state.photoEdit.bodyInfo,
      contentBodyList: state.photoEdit.contentBodyList,
    }));
  ...
  const deleteInfo = async () => dispatch(remove("/content/photo", id));
