...
function SearchPanel({
  ...
  return (
    <Space direction="vertical" size={20}>
      <label key={0} className={styles.titleLabel}>
        {title ? title : "검색"}
      </label>
      <div key={1}>
                  ...
                  <Space size={6}>
                    {subItem.contentItems.map((contentItem, contentIndex) => (
                      <ContentItem
                        item={contentItem}
                        key={contentIndex}
                        value={searchData[contentItem.name]}
      ...
      <Row key={2} justify="center" gutter={[17, 0]}>
        <Col>
          {disableInitButton ? (
            ...
            <Button className="white-button big-button" onClick={onReset}>
              초기화
            </Button>
          )}
        </Col>
        <Col>
          ...
          >
            {actionButtonTitle ? actionButtonTitle : "검색"}
          </Button>
        </Col>
      </Row>
    </Space>
